...
void log_init()
{
    str_t log_file = config_get(CONFIG_LOG_FILE);
    if (log_file == NULL) {
        log_file = DEFAULT_LOG_FILE;
    }
    log_fd = open(str_tocc(log_file), O_APPEND | O_CREAT | O_WRONLY, 0644);
    if (log_fd < 0) {
        log_odebug("log: %r", log_file);
        exit(1);
    }
    log_info("args, config and log init success");
}
...
static str_t log_generate(str_t prefix, int the_errno, str_t fmt, va_list args)
{
    ...
    if (the_errno != 0) {
        // NOT thread-safe, works for multi-processes though
        str_sprintf(&l_errno, "%s: ", strerror(the_errno));
    }
